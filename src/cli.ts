...
import { watchDockerEngine, type DockerEngineTarget } from "./cli/docker-engine.js";
import {
  dockerComposeArgs,
  dockerProjectName,
  isBundledConfig,
...
function createEngineExitHandler(
  bin: string,
  isDocker: boolean,
  readStderr: () => string,
  spawnedAt = Date.now(),
  ...
  return (
    ...
    if (abnormal) {
      ...
      if ((!isDocker || containerStopped) && engineRanFor > ENGINE_STARTUP_GRACE_MS) {
        console.error(
...
function startDockerEngineWatch(
  bin: string,
  target: DockerEngineTarget,
  spawnedAt = Date.now(),
  readStderr: () => string = () => "",
): void {
  const onEngineExit = createEngineExitHandler(bin, true, readStderr, spawnedAt);
  stopDockerEngineWatch?.();
  stopDockerEngineWatch = watchDockerEngine(
    bin,
    target,
    (code) => onEngineExit(code, null, true),
    (error) => {
...
function spawnEngineBackground(
  ...
  let stderrCapture = activeStartupStderr = createStartupStderrCapture();
  ...
  const onEngineExit = createEngineExitHandler(
    bin, isDocker, () => stderrCapture.text(), spawnedAt,
  );
  child.on("exit", (code, signal) => {
    if (isDocker && docker && code === 0) {
      stderrCapture = activeStartupStderr = createStartupStderrCapture();
      startDockerEngineWatch(bin, { ...docker, cwd }, spawnedAt, () => stderrCapture.text());
      return;
    }
    onEngineExit(code, signal);
  });
  child.on("error", (error) => {
    stderrCapture.append(Buffer.from(error.message));
    onEngineExit(1, null);
  });
  child.unref();
  return child;
...
async function startEngine(): Promise<boolean> {
  ...
  if (persistedState?.kind === "docker") {
    ...
    if (inspection.status === "stopped" && !runCommand(
      dockerBin, ["start", inspection.containerId], {
        label: `Restarting owned Docker container ${inspection.containerId}`,
      },
    )) return false;
    startDockerEngineWatch(dockerBin, { containerId: inspection.containerId });
    return true;
  }
...
async function reconcilePersistedDockerEngine(): Promise<boolean> {
  ...
  if (
    !dockerBin ||
    (inspection.status === "stopped" && !runCommand(
      dockerBin, ["start", inspection.containerId], {
        label: `Restarting owned Docker container ${inspection.containerId}`,
      },
    ))
